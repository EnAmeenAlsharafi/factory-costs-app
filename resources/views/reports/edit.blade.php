...
@section('title', 'تعديل تقرير الإنتاج — إدارة تكاليف المصنع')
    ...
    <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 1rem;">
        <div>
            <h1 style="font-size: 1.5rem; margin-bottom: 0.25rem;">
                تعديل تقرير الإنتاج ليوم: {{ $report->production_date->format('Y-m-d') }}
            </h1>
            <p style="color: var(--text-muted); font-size: 0.85rem;">
                آخر تحديث للسجل: {{ $report->updated_at->format('Y-m-d H:i') }} | سيتم إعادة احتساب كافة التكاليف آلياً على الخادم عند الحفظ
            </p>
        </div>
...
<form id="dailyReportForm" action="{{ route('reports.update', $report) }}" method="POST">
    @csrf
    @method('PUT')
        ...
        <div class="card-body" style="padding: 1.25rem;">
            <div style="max-width: 320px;">
                ...
                <input 
                    ...
                    value="{{ old('production_date', $report->production_date->format('Y-m-d')) }}" 
                    required
                >
    ...
    <div class="card desktop-table-view">
        <div class="card-header" style="background-color: #f8fafc;">
            <div style="font-weight: 700;">جدول الأصناف والتعديل (40 صنفاً)</div>
            ...
            <table class="table table-sticky">
                ...
                <tbody>
                    @foreach($products as $index => $product)
                        @php
                            $existingItem = $itemsByProductId->get($product->id);
                            $oldQty = old("items.{$index}.quantity", $existingItem?->quantity ?? 0);
                            $oldStorage = old("items.{$index}.storage_type", $existingItem?->storage_type ?? 'بدون تخزين');
                            $oldCompany = old("items.{$index}.fabric_company_id", $existingItem?->fabric_company_id);
                            $oldType = old("items.{$index}.fabric_type_id", $existingItem?->fabric_type_id);
                        @endphp
    ...
    <div class="card" style="margin-top: 1.5rem; margin-bottom: 5rem;">
        <div class="card-body">
            ...
            <textarea id="notes" name="notes" class="form-control" rows="2" placeholder="أي ملاحظات خاصة بإنتاج هذا اليوم...">{{ old('notes', $report->notes) }}</textarea>
        </div>
    </div>
