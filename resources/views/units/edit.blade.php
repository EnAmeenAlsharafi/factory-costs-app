...
@section('title', 'تعديل وحدة القياس - مصنع مفروشات سدير')
@section('page-title', 'تعديل وحدة القياس: ' . $unit->name_ar)
        ...
        <div class="border-bottom pb-3 mb-4 d-flex justify-content-between align-items-center">
            <div>
                <h5 class="fw-bold text-dark mb-1">
                    <i class="fas fa-edit text-warning me-2"></i> تعديل وحدة القياس
                </h5>
                <p class="text-muted fs-7 mb-0">تحديث مسمى الوحدة، الرمز المختصر أو شروط الكسور العشرية.</p>
            </div>
            <code class="bg-light px-3 py-1.5 rounded-3 text-primary fs-6 fw-bold">{{ $unit->code }}</code>
        </div>

        <form action="{{ route('units.update', $unit) }}" method="POST" x-data="{ allowsDecimal: {{ old('allows_decimal', $unit->allows_decimal) ? 'true' : 'false' }} }">
            @csrf
            @method('PUT')
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" 
                           ...
                           value="{{ old('name_ar', $unit->name_ar) }}" 
                           required>
                    @error('name_ar')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="col-md-6">
                    ...
                    <select name="unit_type" id="unit_type" class="form-select @error('unit_type') is-invalid @enderror">
                        ...
                        <option value="LENGTH" {{ old('unit_type', $unit->unit_type) === 'LENGTH' ? 'selected' : '' }}>أطوال (Length)</option>
                        <option value="AREA" {{ old('unit_type', $unit->unit_type) === 'AREA' ? 'selected' : '' }}>مساحة (Area)</option>
                        <option value="VOLUME" {{ old('unit_type', $unit->unit_type) === 'VOLUME' ? 'selected' : '' }}>حجم (Volume)</option>
                        <option value="WEIGHT" {{ old('unit_type', $unit->unit_type) === 'WEIGHT' ? 'selected' : '' }}>وزن (Weight)</option>
                        <option value="COUNT" {{ old('unit_type', $unit->unit_type) === 'COUNT' ? 'selected' : '' }}>عدد / حبات (Count/Piece)</option>
                        <option value="PACKAGING" {{ old('unit_type', $unit->unit_type) === 'PACKAGING' ? 'selected' : '' }}>تغليف وتعبئة (Packaging)</option>
                    </select>
                    @error('unit_type')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="col-md-6" x-show="allowsDecimal" x-cloak>
                    ...
                    <input type="number" 
                           ...
                           value="{{ old('decimal_precision', $unit->decimal_precision) }}">
                ...
                <div class="col-12 mt-3">
                    <div class="form-check form-switch">
                        <input class="form-check-input" 
                               ...
                               {{ old('allows_decimal', $unit->allows_decimal) ? 'checked' : '' }}>
                        <label class="form-check-label fw-semibold text-dark fs-7" for="allows_decimal">
                            تسمح هذه الوحدة بالكسور العشرية
                        </label>
                    </div>
                </div>
