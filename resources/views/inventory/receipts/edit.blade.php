...
@section('title', "تعديل إيصال استلام: {$receipt->receipt_number}")

@section('content')
<div class="container-fluid py-4" x-data="materialReceiptEditForm()">
    <!-- Header -->
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-1">
                    <li class="breadcrumb-item"><a href="{{ route('inventory.receipts.index') }}" class="text-decoration-none">إيصالات استلام المواد</a></li>
                    <li class="breadcrumb-item"><a href="{{ route('inventory.receipts.show', $receipt) }}" class="text-decoration-none">{{ $receipt->receipt_number }}</a></li>
                    <li class="breadcrumb-item active" aria-current="page">تعديل المسودة</li>
                </ol>
            </nav>
            <h1 class="h3 fw-bold text-dark mb-0">
                <i class="fas fa-edit text-primary me-2"></i>تعديل إيصال استلام خامات (مسودة)
            </h1>
            <span class="badge bg-warning text-dark mt-1 font-monospace">{{ $receipt->receipt_number }}</span>
        </div>
        <div>
            <a href="{{ route('inventory.receipts.show', $receipt) }}" class="btn btn-outline-secondary">
    ...
    <form action="{{ route('inventory.receipts.update', $receipt) }}" method="POST" id="receiptEditForm">
        ...
        <div class="card border-0 shadow-sm rounded-3 mb-4">
            <div class="card-header bg-white py-3 border-0">
                <h5 class="card-title fw-bold mb-0 text-dark"><i class="fas fa-file-invoice text-primary me-2"></i>البيانات الأساسية للإيصال</h5>
            </div>
            <div class="card-body">
                <div class="row g-3">
                    <div class="col-12 col-md-4">
                        ...
                        <select name="supplier_id" id="supplier_id" class="form-select @error('supplier_id') is-invalid @enderror" required>
                            ...
                            @foreach($suppliers as $sup)
                                <option value="{{ $sup->id }}" {{ old('supplier_id', $receipt->supplier_id) == $sup->id ? 'selected' : '' }}>
                                    {{ $sup->name }} ({{ $sup->code }})
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-12 col-md-4">
                        ...
                        <select name="warehouse_id" id="warehouse_id" class="form-select @error('warehouse_id') is-invalid @enderror" required>
                            @foreach($warehouses as $wh)
                                <option value="{{ $wh->id }}" {{ old('warehouse_id', $receipt->warehouse_id) == $wh->id ? 'selected' : '' }}>
                                    {{ $wh->name_ar }} ({{ $wh->code }})
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-12 col-md-4">
                        ...
                        <input type="date" name="receipt_date" id="receipt_date" class="form-control @error('receipt_date') is-invalid @enderror" value="{{ old('receipt_date', $receipt->receipt_date->format('Y-m-d')) }}" required>
                        @error('receipt_date') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>

                    <div class="col-12 col-md-6">
                        <label for="supplier_reference" class="form-label fw-semibold">رقم فاتورة / شحنة المورد</label>
                        <input type="text" name="supplier_reference" id="supplier_reference" class="form-control @error('supplier_reference') is-invalid @enderror" value="{{ old('supplier_reference', $receipt->supplier_reference ?? $receipt->purchase_invoice_reference) }}" placeholder="مثال: INV-99081">
                    ...
                    <div class="col-12 col-md-6">
                        ...
                        <input type="text" name="notes" id="notes" class="form-control @error('notes') is-invalid @enderror" value="{{ old('notes', $receipt->notes) }}" placeholder="أي ملاحظات إضافية...">
...
<script>
function materialReceiptEditForm() {
    return {
        lines: @json($receipt->lines->map(function($l) {
            $isFabric = strtoupper($l->material?->category?->code ?? '') === 'FABRIC';
            return [
                'material_id' => $l->material_id,
                'fabric_color_code' => $l->fabric_color_code ?? $l->fabricColor?->color_code ?? '',
                'is_fabric' => $isFabric,
                'available_colors' => $l->material?->fabricColors?->pluck('color_code')->toArray() ?? [],
                'quantity' => (float) $l->quantity_received,
                'unit_id' => $l->purchase_unit_id,
                'unit_cost' => (float) $l->unit_cost_purchase,
                'lot_reference' => $l->lot_reference ?? '',
            ];
        })),
        addLine() {
