...
@section('title', 'إضافة عميل جديد - مصنع مفروشات سدير')
@section('page-title', 'تسجيل عميل جديد')
    ...
    <div class="card bg-white border-0 shadow-sm rounded-4 p-4 p-md-5">
        <div class="border-bottom pb-3 mb-4">
            <h5 class="fw-bold text-dark mb-1">
                <i class="fas fa-user-plus text-warning me-2"></i> بيانات العميل الجديد
            </h5>
            <p class="text-muted fs-7 mb-0">قم بإدخال بيانات العميل والاتصال وشروط التعامل المالي والائتماني.</p>
        </div>

        <form action="{{ route('customers.store') }}" method="POST" x-data="{ isCredit: {{ old('is_credit_customer') ? 'true' : 'false' }} }">
            @csrf
            ...
            <div class="row g-3">
                ...
                <div class="col-md-4">
                    <label class="form-label text-muted">رمز العميل المقترح</label>
                ...
                <div class="col-md-4">
                    ...
                    <select name="default_sales_channel_id" id="default_sales_channel_id" class="form-select @error('default_sales_channel_id') is-invalid @enderror">
                        ...
                        @foreach ($salesChannels as $channel)
                            <option value="{{ $channel->id }}" {{ old('default_sales_channel_id') == $channel->id ? 'selected' : '' }}>
                                {{ $channel->name_ar }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" 
                           ...
                           value="{{ old('name') }}" 
                           placeholder="مثال: مؤسسة قصر المفروشات للتجارة" 
                           required>
                    @error('name')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" 
                           ...
                           placeholder="مثال: قصر المفروشات">
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" 
                           ...
                           placeholder="اسم مسؤول المشتريات أو العميل">
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" 
                           ...
                           value="{{ old('city') }}" 
                           placeholder="مثال: الرياض، المجمعة، سدير">
                    @error('city')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                    ...
                    <div class="p-3 bg-light rounded-3 border">
                        <div class="form-check form-switch mb-2">
                            <input class="form-check-input" 
                                   ...
                                   value="1" 
                                   x-model="isCredit"
                                   {{ old('is_credit_customer') ? 'checked' : '' }}>
                ...
                <div class="col-12 mt-3">
                    ...
                    <textarea name="notes" 
                              ...
                              placeholder="أي شروط توريد، مواعيد تسليم معتادة، أو تفاصيل خاصة...">{{ old('notes') }}</textarea>
                    @error('notes')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-12 mt-2">
                    <div class="form-check form-switch">
                        ...
                        <label class="form-check-label fw-semibold text-dark fs-7" for="is_active">
                            تفعيل العميل فور التسجيل (إتاحة اختياره في أوامر البيع والفواتير)
                        </label>
                    </div>
                </div>
                ...
                <div class="col-12 d-flex justify-content-end gap-2 mt-4 pt-3 border-top">
                    ...
                    <button type="submit" class="btn btn-factory-warning px-4">
                        <i class="fas fa-save me-1"></i> حفظ بيانات العميل
                    </button>
                </div>
