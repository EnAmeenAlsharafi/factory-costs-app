...
@section('title', 'تعديل قالب تصنيع - ' . $template->template_code)

@section('content')
<div class="container-fluid px-4 py-4" x-data="templateEditForm()">
    ...
    <div class="mb-4">
        ...
        <div class="d-flex align-items-center gap-2">
            <h1 class="h3 mb-1 text-dark fw-bold">تعديل قالب التصنيع</h1>
            <span class="badge bg-dark font-monospace fs-6 px-3 py-1">{{ $template->template_code }}</span>
        </div>
        <p class="text-muted mb-0 fs-7">تعديل بيانات ونمط بنود القالب المسبق</p>
    </div>
    ...
    <form action="{{ route('recipes.templates.update', $template) }}" method="POST">
        @csrf
        @method('PUT')
            ...
            <div class="card-body">
                <div class="row g-3">
                    <div class="col-md-5">
                        ...
                        <input type="text" name="name_ar" class="form-control fw-bold" value="{{ old('name_ar', $template->name_ar) }}" required>
                    </div>
                    <div class="col-md-4">
                        <label class="form-label fw-bold fs-7">اسم القالب باللغة الإنجليزية</label>
                        <input type="text" name="name_en" class="form-control" value="{{ old('name_en', $template->name_en) }}">
                    </div>
                    <div class="col-md-3">
                        <label class="form-label fw-bold fs-7">الحالة</label>
                        <select name="is_active" class="form-select">
                            <option value="1" {{ old('is_active', $template->is_active) ? 'selected' : '' }}>نشط</option>
                            <option value="0" {{ !old('is_active', $template->is_active) ? 'selected' : '' }}>معطل</option>
                        </select>
                    </div>
                    <div class="col-md-12">
                        <label class="form-label fw-bold fs-7">وصف القالب</label>
                        <input type="text" name="description" class="form-control" value="{{ old('description', $template->description) }}" placeholder="وصف استخدام القالب والمواصفات الخشبية">
                    </div>
                </div>
            </div>
        </div>
